<template>
	<div>
		...
		<el-button @click='addAgency()' type="primary" icon="el-icon-check" style="border: 0px;position: relative; left:120px;margin-top: 25px;">确认提交</el-button>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			addAgency(){
				...
				axios.post('/api/agency/agency', parmas, {
						headers: {
							"token": localStorage.getItem("token"),
						}
					})
					.then(res => {
						if(res.data == "AGENCY_ADD_SUCCESS"){
							this.$alert('发送成功', '提示', {
							         confirmButtonText: '确定',
							       });
						}
