...
<script>
	...
	export default {
		...
		methods: {
			getRate() {
				axios.get('/api/rate/personalTimeRate', {
						params: {},
						headers: {
			...
			deleteRate(row) {
				this.$confirm('此操作将取消该利率, 是否继续?', '提示', {
					...
					axios.delete('/api/rate/personalTimeRate', {
							params: {
								id: row.id
							},
							headers: {
			...
			uploaRate(){
				...
				axios.put('/api/rate/personalTimeRate', parmas, {
						headers: {
							"token": localStorage.getItem("token"),
						}
					})
			...
			rateUpdate() {
				...
				parmas.append("month", this.month);
				parmas.append("rate", this.rate);
				axios.post('/api/rate/personalTimeRate', parmas, {
						headers: {
							"token": localStorage.getItem("token"),
						}
					})
			...
			DepositRateUpdate(){
				...
				axios.post('/api/rate/personalDepositRate', parmas, {
						headers: {
							"token": localStorage.getItem("token"),
						}
					})
